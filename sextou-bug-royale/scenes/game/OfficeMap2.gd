...
# 8 Pontos Estratégicos de Spawn no Data Center / Sala de Servidores (2560 x 1440)
const SPAWN_CORNERS: Array[Vector2] = [
	Vector2(260, 240),   # 0: Lab de Hardware (Noroeste)
	Vector2(2300, 240),  # 1: Sala de Nobreaks / UPS (Nordeste)
	Vector2(260, 1200),  # 2: Sala de Refrigeração / Chillers (Sudoeste)
	Vector2(2300, 1200), # 3: Sala de Baterias / Depósito (Sudeste)
	Vector2(1280, 200),  # 4: Corredor Norte / Entrada Principal
	Vector2(1280, 1240), # 5: Corredor Sul / Acesso à Subestação
	Vector2(320, 720),   # 6: Central do NOC / Operações (Oeste)
	Vector2(2240, 720)   # 7: Corredores de Racks / Fileira Leste
]
...
var match_duration: float = 180.0 # 3 minutos até o Happy Hour das 18h
...
func _update_reboot_zone_status() -> void:
	if is_game_over or info_label == null:
		return
...
func get_valid_loot_spawn(calling_box: Node2D) -> Vector2:
	var candidate_slots: Array[Vector2] = [
		Vector2(450, 320), Vector2(750, 320), Vector2(1820, 320), Vector2(2120, 320),
		Vector2(450, 620), Vector2(720, 720), Vector2(1820, 620), Vector2(2120, 620),
		Vector2(1100, 620), Vector2(1460, 620), Vector2(1100, 820), Vector2(1460, 820),
		Vector2(450, 1120), Vector2(750, 1120), Vector2(1820, 1120), Vector2(2120, 1120)
	]
...
func _setup_match() -> void:
	...
	if remote_players.is_empty():
		var bot_names = ["Dev_Bugger", "QA_Hunter", "Bug_Slayer", "SysAdmin_Zero", "Root_Destroyer", "Deploy_Master", "Docker_Daemon"]
		for i in range(min(3, bot_names.size())):
...
func _show_game_over_banner(message: String, is_victory: bool) -> void:
	if not is_victory:
		...
		flash_overlay.visible = true
		flash_overlay.color = Color(0.95, 0.85, 0.2, 0.5)
		var tween = create_tween()
